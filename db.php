...
class Database {
    ...
    // 优化：添加最小保存间隔，避免过于频繁的数据库写入
    public function savePlayerHistory($server_id, $players_online, $player_list = null) {
        // 检查上一条记录的时间，如果距离现在太近（小于30秒），则跳过保存
        $sql_check_time = "SELECT record_time FROM player_history WHERE server_id = ? ORDER BY record_time DESC LIMIT 1";
        $stmt_check_time = $this->conn->prepare($sql_check_time);
        
        if ($stmt_check_time === false) {
            error_log("查询最后一条记录时间prepare语句失败: " . $this->conn->error);
            return false;
        }
        
        $stmt_check_time->bind_param("i", $server_id);
        $stmt_check_time->execute();
        $result_check_time = $stmt_check_time->get_result();
        $stmt_check_time->close();
        
        if ($result_check_time && $result_check_time->num_rows > 0) {
            $row_time = $result_check_time->fetch_assoc();
            $last_record_time = strtotime($row_time['record_time']);
            $time_diff = time() - $last_record_time;
            
            // 如果距离上次保存小于30秒，跳过这次保存
            if ($time_diff < 30) {
                return true; // 返回成功，但不实际保存
            }
        }
