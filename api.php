...
class MinecraftAPI {
    ...
    public function getServersStatusInParallel($servers) {
        ...
        foreach ($servers as $server) {
            ...
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_USERAGENT, 'Minecraft-Server-Status-Monitor/1.0');
            curl_setopt($ch, CURLOPT_TIMEOUT, 5); // 5秒超时（从10秒减少）
            curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 3); // 3秒连接超时（从5秒减少）
    ...
    public function handleRequest() {
        ...
        switch ($action) {
            ...
            case 'save_player_history':
                if (isset($_GET['server_id']) && isset($_GET['players_online'])) {
                    $server_id = $_GET['server_id'];
                    $players_online = $_GET['players_online'];
                    $player_list = isset($_GET['player_list']) ? json_decode($_GET['player_list'], true) : null;
                    
                    $result = $this->db->savePlayerHistory($server_id, $players_online, $player_list);
                    if ($result) {
                        $this->sendSuccessResponse(['message' => '历史数据保存成功']);
                    } else {
                        $this->sendErrorResponse('保存历史数据失败');
                    }
                } else {
                    $this->sendErrorResponse('缺少必要参数');
                }
                break;
